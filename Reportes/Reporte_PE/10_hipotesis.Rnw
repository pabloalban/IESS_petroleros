...
En la tabla \ref{tab:iess_hip_macro} se presentan los parámetros macroeconómicos estimados antes de la pandemia para el período 2020 a 2060
...
Debido a la crisis económica generada por el COVID-19, se actualizan: la tasa de crecimiento de los salarios, la tasa de crecimiento del salario básico unificado y el crecimiento del PIB en la siguiente tabla \ref{tab:iess_hipos_covid}.

\begin{table}[H]
 \centering
\caption{Nuevas hipótesis en el escenario Post-COVID}
 \label{tab:iess_hipos_covid}
\begin{tabular}{@{}cccc@{}}
\toprule
Año              & Tasa variación salarial & Tasa variación SBU & Tasa variación PIB \\ \midrule
2019             & 3,33\%                  & 2,07\%             & 0,05\%             \\
2020             & 0,00\%                  & 1,52\%             & -7,27\%            \\
2021             & 0,72\%                  & 1,86\%             & 4,10\%             \\
2022             & 1,44\%                  & 2,20\%             & 1,67\%             \\
2023 en adelante & 2,15\%                  & 2,53\%             & 1,67\%             \\ \bottomrule
\end{tabular}
\end{table}
...
La reserva inicial del \Sexpr{REP_seg} empleada para el cálculo de la reserva en términos corrientes, $V_t^{cap}$, y la reserva en valor presente, $V_t$, corresponde al patrimonio al 31 de diciembre de 2018 (ver tabla \ref{tab:iess_patrimonio_fondo_rtr}), el cual asciende a USD \Sexpr{REP$cap_ini}.
